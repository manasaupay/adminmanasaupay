    ...
    <section className="glass-card rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-base font-black text-slate-900">Create Partner Login & Profile</h2>
        ...
        <button
          ...
          className="rounded-xl bg-teal-600 px-5 py-3 text-xs font-black tracking-wide text-white hover:bg-teal-700 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 transition-all duration-150 shadow-sm cursor-pointer"
        >
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <div className="space-y-1.5 col-span-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:border-teal-500/40 focus:ring-1 focus:ring-teal-500/20 outline-none transition-all"
          />
        </div>
      ...
      {assignment && (
        <div className="mt-5 space-y-2 border-t border-slate-100 pt-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <p className="text-xs font-bold text-slate-900">Profile Assignment Linkage</p>
            ...
            <select
              ...
              className="w-full sm:w-80 rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs font-bold text-teal-600 focus:border-teal-500/40 outline-none transition-all cursor-pointer"
              required
            >
              <option value="" className="bg-white text-slate-400">-- Link {assignment.label} --</option>
              <option value="create_new" className="bg-white text-teal-600 font-bold">
          ...
          {form.assignedTargetId === "create_new" && (
            <div className="mt-3 p-5 rounded-2xl border border-teal-100 bg-teal-50/40 grid gap-4 md:grid-cols-3 shadow-sm">
              <div className="col-span-3 pb-1 border-b border-teal-100">
                <p className="text-xs font-black text-teal-700 uppercase tracking-wider flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 rounded-full bg-teal-500 glow-active shadow-[0_0_8px_rgba(20,184,166,0.3)]" />
                ...
                <>
                  <div className="space-y-1.5 col-span-1">
                    ...
                    <input
                      ...
                      placeholder="e.g. Sharma Repairs (defaults to partner)"
                  ...
                  <div className="space-y-1.5 col-span-1">
                    ...
                    {assignment.categories.length === 0 ? (
                      <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-2.5 text-[10px] text-red-600 leading-normal font-bold">
                        ⚠️ No categories found for <strong className="text-red-700">"{form.role === 'service_provider' ? 'services' : 'businesses'}"</strong>.
                        <br />
                        <Link href="/add-new" className="text-teal-600 font-bold underline hover:text-teal-700 inline-block mt-1">
                      ...
                      <select
                        ...
                        className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs font-semibold text-slate-800 focus:border-teal-500/40 outline-none cursor-pointer"
                        required
                      >
                  ...
                  <div className="space-y-1.5 col-span-1">
                    ...
                    >
                      ...
                      {subcategories.map((sub: any) => (
                        <option key={sub.value} value={sub.value} className="bg-white text-slate-850">
                          {sub.label}
                        </option>
                      ))}
                    </select>
      ...
      {assignment && assignment.options.length === 0 && form.assignedTargetId !== "create_new" && (
        <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 flex items-center gap-3">
          ...
          <p className="text-[11px] text-amber-700 font-bold leading-normal">
            No unassigned profiles are currently available. We highly recommend selecting <strong className="text-slate-900">"+ Create New {assignment.label}"</strong> in the link selector above to automatically create and bind one.
          </p>
        </div>
      )}
      ...
      {/* Response Banners */}
      {message && (
        <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-xs font-bold text-emerald-700 flex items-center gap-2.5 shadow-sm">
      ...
      {error && (
        <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-4 text-xs font-bold text-red-700 flex items-center gap-2.5 shadow-sm">
